            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu" style="max-height: 100px; overflow-y: scroll;">
                    <li v-for="(country_Code, index) in country_Code_Numbers()"
                        @click="model_Update(country_Code_Flag()[index], this.phone_Text, index)"><a class="dropdown-item" href="#"><img
                                :src="country_Code_Flag()[index]">{{ country_Code }}</a>
                    </li>
                </ul>
            </div>
        </div>
        <input type="text" class="form-control" aria-label="Text input with dropdown button" v-model="phone_Text">
    </div>
    </div>
</template>
